...
// require.context('./views', true, /[^components]\/index\.vue$/)
const views = require.context('./views', true, /^.+[^(\/component(\/\w+)*)]\/index\.vue$/).keys()
console.log(views);
...
const app = new Vue({
  ...
  router: new VueRouter({
    ...
    routes: require.context('./views', true, /[^(component(\/\S+)*\/\w+\.vue)]\/index\.vue$/)
    .keys()
    .reduce((res, v)=> v.split('/').slice(1, -1)
    .reduce((r,c,i,a)=> {
      const namne = a.slice(0, i + 1).join('.');
      let t = r.length && r.find(({name: N}) => name === N) || null;
      if (!t) {
        t = {
          path: (!i ? '/' : '') + c,
          name
